...
Assuming we created the workspace as described earlier, we now create the first project with the command `cargo new schotter1`. This will create the folder "schotter1" with a "src" subfolder and the project Cargo.toml file. It will also add "schotter1" as a workspace member in the workspace Cargo.toml file.

The next step is to make that a nannou project by editing the project Cargo.toml file (the one in the schotter1 folder, not the workspace one we modified earlier) to add nannou as a dependency. We'll use the version specified in the workspace Cargo.toml, so we add the following to the end of schotter1/Cargo.toml, after `[dependencies]`:
...
The final setup step is to compile the template using the command `cargo run -p schotter1`. This will take a few minutes the first time it is run since it needs to compile nannou and all of the libraries that it uses. Subsequent compiles will complete much more quickly.

When it's finished, a window will appear containing a blue circle on a plum background. Not very exciting, but it shows that the environment is working. Close the window to exit.
...
Running this (with `cargo run -p schotter1`), we see one square at the top left, just like we expect.
...
But running it now produces a surprise: the squares jump around very rapidly. This happens because `view` is run 60 times a second, and new random positions are chosen each time. We only want to loop once. Unfortunately, nannou version 0.20.0 doesn't have this capability. It has been added for future versions; simply add `.loop_once()` to the sketch builder:

```
nannou::sketch(view).size(WIDTH, HEIGHT).loop_once().run();
```

But as a workaround for now, we can stop the loop by adding the following line to the beginning of `view`:

```
app.set_update_mode(UpdateMode::wait());
```

In wait mode, the frame loop pauses until an event is received. This includes mouse events, so moving the mouse in the window will make the squares move randomly.

And we're done! The result is, of course, different from what Georg Nees got; indeed it will be different each time the program is run. That's the fun of generative art.
