...
*Schotter* is fairly simple to code; at least it is today with modern graphics libraries. Several other people have written tutorials for how to code this using various platforms. I'm adding to this collection with a series of tutorials for coding it in Rust using the [nannou](https://nannou.cc/) library.
...
2. Create a new file named "Cargo.toml" in that directory with the following contents:
    ...
    opt-level = 3
    ```
The "members" is a list of workspace members; we'll add our variations to it as we go. Nannou requires it to be at least "2", but we'll use the latest: "3".

The "workspace.dependencies" section lists the common dependencies for the projects in the workspace. Specifying the version of nannou here allows changing it in a single place when we need to change it.
...
```
nannou = "0.20.0"             # A Creative Coding Framework for Rust.
```

When I first wrote this in May 2021, the latest version was 0.16.0, but a new version was released the next month so I updated this tutorial to use 0.17.0. This actually required changing some of the code since the parameters for the random number generator function gen_range() changed (we'll see this in schotter2). When version 0.18.0 was released five months later, yet more changes were needed. And when version 0.20.0 was released five years later, even more changes were needed. Specifying the exact version of the libraries in Cargo.toml prevents changes from breaking our code when libraries are updated.

The "profile" sections determine what optimization level to use. The first one specifies a low optimization level for the programs in our workspace and the other specifies a high optimization level for the packages it uses (such as nannou). This allows debugging to be used for our programs but makes nannou run much faster.
...
**Note:** This tutorial was updated for nannou version 0.20.0, released in June 2026. This was a major update, rewriting it to use the Bevy game engine. And there are a few glitches in the first version using Bevy. The ones that affect the tutorial are described along with workarounds to make the tutorial work. 